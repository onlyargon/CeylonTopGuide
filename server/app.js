...
const allowedOrigins = [
	...
	'https://www.ceylontopguide.com',
	'https://ceylontopguide.com',
];
...
guideRouter.post('/login', async (req, res) => {
	try {
		...
		if (!guide.account.isVerified) {
			return res
				.status(403)
				.json({ error: 'Your account is awaiting approval.' });
		}
		...
		res
			.status(200)
			.json({ message: 'Login Successful!', guide: req.session.guide });
	} catch (error) {
