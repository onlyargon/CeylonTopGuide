    ...
    <div className="guide-list-container">
      <div className="sidebar">
        ...
        <div className="filter-group">
          <label>Rank</label>
          {guideRanks.map(rank => (
            <div key={rank}><input type="radio" name="rank" value={rank} onChange={handleChange} checked={filters.rank === rank}/> {rank}</div>
          ))}
        </div>
        ...
        <div className="filter-group">
          ...
          {languages.map(lang => (
            <div key={lang}><input type="radio" name="language" value={lang} onChange={handleChange} checked={filters.language === lang}/> {lang}</div>
          ))}
        </div>
        ...
        <div className="filter-group">
          ...
          <select name="experience" value={filters.experience} onChange={handleChange}>
            {experienceOptions.map(exp => <option key={exp} value={exp}>{exp === '10+' ? '10+ years' : `${exp} years`}</option>)}
          </select>
        </div>
        ...
        <div className="filter-group">
          ...
          <select name="region" value={filters.region} onChange={handleChange}>
            {tourRegions.map(r => <option key={r} value={r}>{r}</option>)}
          </select>
        </div>
        ...
        <div className="filter-group">
          ...
          <select name="availability" value={filters.availability} onChange={handleChange}>
            {availableDays.map(a => <option key={a} value={a}>{a}</option>)}
          </select>
        </div>

        <div className="filter-group">
          <label>Hourly Rate (Max)</label>
          <input type="number" name="maxHourlyRate" value={filters.maxHourlyRate} onChange={handleChange} />
        </div>

        <div className="filter-group">
          <label>Daily Rate (Max)</label>
          <input type="number" name="maxDailyRate" value={filters.maxDailyRate} onChange={handleChange} />
        </div>
